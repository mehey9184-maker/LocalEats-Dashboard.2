...
- The staged Task 1B database contract standardizes order and rider-connection shop IDs as required text foreign keys, maps Firebase riders through a required unique `firebase_uid`, and defaults new rider profiles and merchant-rider connections to `pending`. Browser roles have no direct authority over orders or rider authority tables; the LocalEats API service role is the sole staged access path.
- Rider claims remain atomic and require an online approved rider plus an explicitly `approved` merchant connection. Legacy `active` connection state no longer grants claim authority.
- Delivery completion uses separate PIN/QR proof kinds. Invalid PIN attempts persist in the order row, lock after five failures for 15 minutes, and reset with both proof hashes after success. Completion increments factual delivery count once but does not award earnings, points, or create a cash-settlement rule.
- Completed or cancelled customer orders no longer return regenerated active delivery confirmation credentials.
...
- LR1A lifecycle interpretation accepts only these persisted `status + delivery_status` pairs: `pending + none`, `preparing + none`, `ready_for_pickup + none/finding_rider/rider_assigned/picked_up/delivering`, and `delivered + delivered`. Every other pair (including cancelled, collected, unknown, null, or contradictory states) fails closed with `409 INVALID_ORDER_STATE`; delivered remains terminal under the unchanged transition table. Ownership checks and compare-and-set updates are unchanged. This local guard does not repair existing data, merchant direct-write bypasses, or Rider split-brain behavior.
- LR1B makes Merchant order lifecycle writes API-authoritative in the browser. Accept and ready actions await the authenticated Merchant API and replace UI state only with the server-confirmed order before refreshing. Direct browser helpers for `orders.status`, `orders.delivery_status`, and `orders.rider_id`, legacy delivery-status controls, stale order-cache hydration, and local lifecycle overrides are retired. Offline `UPDATE_ORDER` entries are discarded with a warning and are neither applied to Firestore nor counted as synchronized. Cancel, complete, delivery-stage, direct assignment/reassignment, and pickup-conversion controls remain removed or fail closed until dedicated server-authorized endpoints exist. Rating, POS creation, and Danger Zone deletion remain separately deferred authority risks.
- LR1C validates the complete persisted `status + delivery_status` pair for every Rider lifecycle mutation. Claim requires `ready_for_pickup + finding_rider` with no assigned rider; pickup requires `ready_for_pickup + rider_assigned`; delivering requires `ready_for_pickup + picked_up`; completion requires `ready_for_pickup + delivering`, and replay is valid only for `delivered + delivered`. Rider reads filter by the same canonical pairs, pickup/delivering use a persisted pre-read plus compare-and-set guards, and contradictory data fails closed without proof mutation or rider-stat increments. The claim/completion RPC hardening remains staged migration code only and has not been applied.
- LR1D completes the Merchant-controlled pilot lifecycle through authenticated API actions only. Reject changes unassigned `pending + none` to `cancelled + none`; Merchant cancel changes only unassigned `preparing + none` to `cancelled + none`; and collection confirmation changes only an unassigned collection order from `ready_for_pickup + none` to `collected + none`. All three use the loaded canonical source pair and a guarded Supabase server update; Merchant cancellation stops before ready/rider search, while delivery completion remains Rider authority. `collected + none`, `delivered + delivered`, and `cancelled + none` are canonical terminal states, and the Merchant UI classifies them as history without direct browser, Firestore, offline, or local-success lifecycle writes. No migration was applied.
- LR1E hardens the shared authoritative quote/create validation path: ordering requires an approved, active, non-archived shop, so archived shops fail closed before pricing or insertion. Customer delivery coordinates must be finite JSON numbers within geographic ranges. Shop delivery coordinates also fail closed when missing or invalid and are selected only as complete pairs: `lat/lng` is preferred when both are present, while `latitude/longitude` is used only when both canonical values are absent and both legacy values are present; coordinate families are never mixed. The server-authoritative 4 km pilot radius is unchanged. No migration was applied and no live database data was changed.
- LR1-F-A adds server-enforced customer price consent for new orders without transferring pricing authority to the client. Quote remains server-calculated, read-only, and does not require or use `accepted_total_price`; create recalculates the same authoritative price and uses the consent value only as a comparison precondition. Consent is converted to whole cents from the number's canonical decimal representation, rejecting coercion, unsafe cents, or more than two decimal places without a magnitude-based tolerance. Explicit top-level and item-level client pricing breakdowns fail with `CLIENT_PRICING_REJECTED`. A new order without consent fails with `PRICE_CONSENT_REQUIRED`, and changed consent fails with `PRICE_CHANGED`, before proof creation or insertion. Consent is not persisted, existing idempotent replay/conflict behavior remains intact, and no migration, live database change, or deployment occurred.
