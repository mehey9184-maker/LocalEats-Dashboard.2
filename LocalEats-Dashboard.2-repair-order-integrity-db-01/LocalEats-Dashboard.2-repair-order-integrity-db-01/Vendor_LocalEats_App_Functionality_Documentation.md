...
## 11. Public Catalog API (Catalog-01)
- public read-only catalog API exists
- endpoints:
  GET /api/v1/catalog/shops
  GET /api/v1/catalog/shops/:shopId
  GET /api/v1/catalog/shops/:shopId/menu
- Supabase is authoritative
- only approved + unarchived shops are public
- approved inactive shops remain visible as offline
- orderability is still protected by existing approved + is_active server checks
- catalog API does not read Firestore
- no live migration was required
